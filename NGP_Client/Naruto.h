...
glm::vec3 narutoCoord(2.25f, 0.0f, 0.0f);
...
void drawNaruto(GLuint program, int vertexCount, GLuint vao[], GLuint vbo[],glm::mat4 viewMatrix, glm::mat4 projectionMatrix) {
    ...
    transMatrixpoint = glm::translate(transMatrixpoint, glm::vec3(narutoCoord.x, narutoCoord.y, narutoCoord.z));
...
void spiderNaruto() {
    if (!map2) {
        ...
        if (leftWire) {
            setaLefthand = -90.0f;
            setam = 135.0f;
            wireCoord[0] = glm::vec3(narutoCoord.x + 0.45, narutoCoord.y + 2.9, narutoCoord.z - 0.3);
...
void gravityNaruto() {
    if (!boolj2) {
        if (!map2) {
            boolgravity = true;
            for (int i = 0; i < BUILDINGNUMBER; i++) {
                if (handrailCoord[i].z - (nangansize/2.0) < narutoCoord.z && narutoCoord.z < handrailCoord[i].z + (nangansize / 2.0)) { //캐릭터가 건물 난간위에 있을때
                    if (handrailCoord[i].x > 0) {
                        if (narutoCoord.x >= 2.25-0.6 && narutoCoord.x <= 2.25 + 0.6) {
                            boolgravity = false;
                        }
                    }
                    else {
            ...
            for (int i = 0; i < 1; i++) {
                if (handrailCoord2[i].x - 3.0f < narutoCoord.x && narutoCoord.x < handrailCoord2[i].x + 3.0f) { //캐릭터가 건물 난간위에 있을때
                    if (handrailCoord3[i].z > -11.5 * 6 - 8) {
                        if (narutoCoord.z <= -11.5 * 6 - 4.5 + 0.3 && narutoCoord.z >= -11.5 * 6 - 5.75 - 0.6) {
                            boolgravity = false;
                        }
                    }
                }
            ...
            for (int i = 0; i < 1; i++) {
                if (handrailCoord2[i].x - 3.0f < narutoCoord.x && narutoCoord.x < handrailCoord2[i].x + 3.0f) { //캐릭터가 건물 난간위에 있을때
                    if (handrailCoord3[i].z > -11.5 * 6 - 8) {
                        if (narutoCoord.z <= -11.5 * 6 - 5.75 + 0.6 && narutoCoord.z >= -11.5 * 6 - 5.75 - 0.6) {
                            boolgravity = false;
                        }
                    }
                    else {
                        if (narutoCoord.z <= -11.5 * 6 - 10.25 + 0.6 && narutoCoord.z >= -11.5 * 6 - 10.25 - 0.6) {
                            boolgravity = false;
                        }
                    }
                }
            ...
            for (int i = 0; i < BUILDINGNUMBER3; i++) {
                if (handrailCoord3[i].x - (nangansize / 2.0) < narutoCoord.x && narutoCoord.x < handrailCoord3[i].x + (nangansize / 2.0)) { //캐릭터가 건물 난간위에 있을때
                    if (handrailCoord3[i].z > -11.5*6-8) {
                        if (narutoCoord.z <= -11.5*6-5.75+0.6 && narutoCoord.z >= -11.5 * 6 - 5.75 - 0.6) {
                            boolgravity = false;
                        }
            ...
            if (clear[0].x - 1.5f < narutoCoord.x && narutoCoord.x < clear[0].x + 1.5f ) {
                boolgravity = false;
                boolw = false;
                gameclear = true;
                if (version3) {
